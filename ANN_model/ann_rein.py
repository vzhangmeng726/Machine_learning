...
class NeuralNetworkClassifier(object):
    ...
    def __init__(self, input_count, output_count, hidden_layers = None, lmbd = 1.0,
        ...
        self.layers = [input_count]
        self.layers.extend(self.hidden_layers)
        self.layers.append(output_count)
...
#        self.labels_index_map = dict(zip(range(labels_count), target_labels))
        ...
        self.target = np.zeros((X.shape[0], self.output_count))
        for i, label in enumerate(y):
             self.target[i, label] = 1

#            self.target[i, self.labels_map[label]] = 1

        self.result = optimize.minimize(self._f, x0 = self._theta,
